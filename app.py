...
def get_pro_data(tf):
    ...
    # Fetch cross rates (we normalize everything to INR first, then convert to display currency)
...
try:
    ...
    # 3. GLOBAL CONVERSION FACTOR
    # Logic: 1 INR = (1 / rate_of_display_currency) in display currency
    ...
    for acc_name, holdings in PORTFOLIO_DATA.items():
        acc_daily_val_inr = pd.Series(0, index=hist.index)
        ...
        for h in holdings:
            ...
            # Keep historical series in INR for normalization consistency
    ...
    m1.metric(f"Folio Value ({display_currency})", f"{sym}{total_val:,.0f}")
    
    total_pnl_sum = df[f'Total P&L ({display_currency})'].sum()
    total_inv = total_val - total_pnl_sum
    m2.metric("Total Unrealized P&L", f"{sym}{total_pnl_sum:,.0f}", f"{(total_pnl_sum/total_inv*100):.2f}%")
    
    d_pnl = df[f"Day P&L ({display_currency})"].sum()
    m3.metric("Today's P&L", f"{sym}{d_pnl:,.0f}", f"{(d_pnl/total_val*100):.2f}%")
    ...
    with col_left:
        ...
        fig_sun = px.sunburst(df, path=['Account', 'Symbol'], values=f'Value ({display_currency})',
